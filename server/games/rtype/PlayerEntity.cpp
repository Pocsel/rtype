...
namespace Gmgp
{
    namespace Server
    {
        ...
        PlayerEntity::PlayerEntity(Game& game, uint16_t playerId, String const& nick) :
            GameEntity(game, "Player", playerId), _ship(0), _scoreboardOpen(false)
        {
            if (this->_game.GetMode() == MainGame::ENDURANCE)
            {
                this->SpawnShip();
                this->_ship->InfoText(nick + " joined");
            }
        ...
        PlayerEntity::~PlayerEntity()
        {
            ...
            if (this->_ship != 0)
            {
                try
                {
                    this->_ship->InfoText(this->_game.GetPlayerManager().GetPlayer(this->GetPlayerId()).GetNick() + " left");
                }
                catch (std::exception&)
                {
                }
                delete this->_ship;
                this->_game.GetEntityManager().GetInteractionManager().AddInteraction(InteractionPtr(new ForceDowngradeInteraction(this->GetPlayerId(), 0)),
                                                                                      Circle(0, 0, 2048));
            }
            this->_game.UnregisterPlayerEntity(this);
        }

        bool PlayerEntity::HasShip() const
        {
            return this->_ship != 0;
        }

        void PlayerEntity::SpawnShip()
        {
            this->_ship = new ShipEntity(this->_game, this->GetPlayerId());
            if (this->_game.GetMode() == MainGame::NORMAL)
            {
                try
                {
                    this->_ship->InfoText(this->_game.GetPlayerManager().GetPlayer(this->GetPlayerId()).GetNick());
                }
                catch (std::exception&)
                {
                }
